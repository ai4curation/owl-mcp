...
import tempfile
import pytest
from owl_mcp.xml_catalog_utils import read_catalog, find_catalog_file, resolve_iri, load_catalog_for_ontology
...
<catalog prefer="public" xmlns="urn:oasis:names:tc:entity:xmlns:xml:catalog">
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert mappings["http://example.org/ontology.owl"] == os.path.join(tmpdir, "local", "ontology.owl")
        assert mappings["http://example.org/imports/base.owl"] == os.path.join(tmpdir, "imports", "base.owl")
        assert mappings["http://test.org/test.owl"] == os.path.normpath(os.path.join(tmpdir, "..", "test.owl"))
